...
import weilan.app.activity.BindingAcitivity;
...
import weilan.app.zbar.lib.camera.CameraManager;
import weilan.app.zbar.lib.decode.CaptureActivityHandler;
import weilan.app.zbar.lib.decode.InactivityTimer;
...
public class CaptureActivity extends Activity implements Callback {
			...
			if (action.equals(BCH_CONSTANT.ACTION)) {
				...
				if (message.equals("check_success")) {
				    Intent mintent = new Intent();
					 mintent.putExtra("deviceNumber", deviceNumber);
					 mintent.setClass(CaptureActivity.this,
					 BindingAcitivity.class);
					 startActivity(mintent);
				
				} else if (message.equals("check_fail")) {
					Toast.makeText(CaptureActivity.this, "设备未入网或已绑定主人",
	...
	public void handleDecode(String result) {
		...
		deviceNumber = result;
		boolean isSend = sendToSevice("check", username, null, deviceNumber, null);
		if (!isSend) {
			Toast.makeText(CaptureActivity.this, "搜索失败", Toast.LENGTH_SHORT)
					.show();
		} else {
			//finish();
		}

		// 连续扫描，不发�?此消息扫描一次结束后就不能再次扫�?		// handler.sendEmptyMessage(R.id.restart_preview);
	}
