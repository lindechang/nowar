...
import com.nowar.db.DB_CONSTANT.TABLES.MSGEX;
...
public class CreateDB {
	...
	// ClientListener.sharedPreferences.getString("userName", "")+"_DB";
	...
	private final static String CREATE_TABLE_SWdevice = "CREATE TABLE IF NOT EXISTS "
			...
			+ BtDB.SWDEVICE_NAME
			+ " TEXT,"
			+ BtDB.SWDEVICE_NUMBER
			+ " TEXT,"
			+ BtDB.SWDEVICE_SELECT
			+ " INTEGER," + BtDB.SWDEVICE_SET + " INTEGER)";
	...
	private final static String CREATE_TABLE_book = "CREATE TABLE IF NOT EXISTS "
			...
			+ " TEXT," + FriendTable.FRIEND_NUMBER + " TEXT)";
	...
	private final static String CREATE_TABLE_MSGEX = "CREATE TABLE IF NOT EXISTS "
			...
			+ " TEXT,"
			+ MSGEX.FIELDS.MSGEX_RESERVED + " TEXT)";

	public CreateDB(Context ctx) {
		mCtx = ctx;
		DATABASE_NAME = SharedPrefsUtil.getSharedPrefsUtil().getValue(mCtx,
				"userName", "")
				+ "_DB";
